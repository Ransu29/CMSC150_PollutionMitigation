...
BuildInitialMatrix<-function(chosenProjectNames){
  ...
  numOfProjectConstraints = -1*diag(numOfProjects)
  rownames(numOfProjectConstraints) = chosenProjectNames
  
  #adds it to the matrix
  chosenProjects = rbind(chosenProjects, numOfProjectConstraints)
  chosenProjects = rbind(chosenProjects, zRow)
  
  RHS = c(target_values, maxProjConstraint, 1)
  chosenProjects = cbind(chosenProjects, RHS)
...
BuildTableau <- function(transposedMatrix){
  ...
  transposedMatrix[rows, ] = -1 * transposedMatrix[rows, ]
  # makes the RHS of the last row 0
  transposedMatrix[rows, cols] = 0
  ...
  colnames(tableau) <- c(variableColsNames, colnames(matrix_part1), "Z", "RHS")
  rownames(tableau) <- rownames(transposedMatrix)
  
  # adjust bottom-right cell if needed
  tableau[nrow(tableau), (ncol(tableau) - 1)] = 1
  return(tableau)
}

proj = c("Large Solar Park", "Small Solar Installations", "Wind Farm", "Gas-to-renewables conversion", "Boiler Retrofit", 
